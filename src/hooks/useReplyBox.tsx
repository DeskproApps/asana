...
export type SetSelectionState = (taskId: Task["gid"], selected: boolean, type: ReplyBoxType) => void|Promise<{ isSuccess: boolean }|void>;

export type GetSelectionState = (taskId: Task["gid"], type: ReplyBoxType) => void|Promise<Array<GetStateResponse<string>>>;

export type DeleteSelectionState = (taskId: Task["gid"], type: ReplyBoxType) => void|Promise<boolean|void>;
...
const noteKey = (ticketId: string, taskId: Task["gid"]|"*") => {
  return `tickets/${ticketId}/${APP_PREFIX}/notes/selection/${taskId}`.toLowerCase();
};

const emailKey = (ticketId: string, taskId: Task["gid"]|"*") => {
...
const registerReplyBoxNotesAdditionsTargetAction = (
  ...
  taskIds: Array<Task["gid"]>,
  tasksMap: Record<Task["gid"], Task>,
): void|Promise<void> => {
  if (!ticketId) {
    return;
  }
  ...
  return Promise
    .all(taskIds.map((taskId: Task["gid"]) => client.getState<{ selected: boolean }>(noteKey(ticketId, taskId))))
    .then((flags) => {
      client.registerTargetAction(`${APP_PREFIX}ReplyBoxNoteAdditions`, "reply_box_note_item_selection", {
        title: "Add to Asana",
        payload: taskIds.map((taskId, idx) => ({
          id: taskId,
          title: has(tasksMap, [taskId, "name"]) ? truncate(tasksMap[taskId].name, { length: 20 }) : taskId,
...
const registerReplyBoxEmailsAdditionsTargetAction = (
  ...
  return Promise
    .all(taskIds.map((taskId: Task["gid"]) => {
    ...
    .then((flags) => {
      return client.registerTargetAction(`${APP_PREFIX}ReplyBoxEmailAdditions`, "reply_box_email_item_selection", {
        ...
        payload: taskIds.map((taskId, idx) => ({
          id: taskId,
          title: has(tasksMap, [taskId, "name"]) ? truncate(tasksMap[taskId].name, { length: 25 }) : taskId,
          selected: flags[idx][0]?.data?.selected ?? false,
        })),
      });
    });
...
const ReplyBoxProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const tasksMap = useMemo(() => (Array.isArray(tasks) ? tasks : []).reduce<Record<Task["gid"], Task>>((acc, task) => {
    acc[task.gid] = task;
    return acc;
  }, {}), [tasks]);
  ...
  useInitialisedDeskproAppClient((client) => {
    if (isCommentOnNote) {
      registerReplyBoxNotesAdditionsTargetAction(client, ticketId, map(tasks, "gid"), tasksMap);
      client.registerTargetAction(`${APP_PREFIX}OnReplyBoxNote`, "on_reply_box_note");
    }

    if (isCommentOnEmail) {
      registerReplyBoxEmailsAdditionsTargetAction(client, ticketId, map(tasks, "gid"), tasksMap);
        ...
        client.getState<{ id: string; selected: boolean }>(emailKey(ticketId, "*"))
          .then((selections) => {
            const taskIds = selections
              ...
              .map(({ data }) => data?.id as Task["gid"]);

            return Promise.all(taskIds.map((taskId) => createTaskCommentService(client, taskId, { text: email })))
          })
      ...
      .with(`${APP_PREFIX}OnReplyBoxNote`, () => {
        ...
        client.getState<{ id: string; selected: boolean }>(noteKey(ticketId, "*"))
          .then((selections) => {
            ...
            return Promise.all(taskIds.map((taskId) => createTaskCommentService(client, taskId, { text: note })));
          })
          .finally(() => client.setBlocking(false));
      })
        ...
        (action.payload ?? []).forEach((selection: { id: string; selected: boolean; }) => {
          ...
          if (ticketId) {
            client?.setState(noteKey(subjectTicketId, selection.id), { id: selection.id, selected: selection.selected })
              .then((result) => {
                if (result.isSuccess) {
                  registerReplyBoxNotesAdditionsTargetAction(client, subjectTicketId, map(tasks, "gid"), tasksMap);
                }
              });
          }
        })
